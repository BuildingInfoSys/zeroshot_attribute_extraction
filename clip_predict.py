...
def predict(model, text_input, img_paths, preprocess, device, batch_size = 200, agg = "max", num_classes = 3):
    ...
    for batch_idx in tqdm(range(len(img_paths)//batch_size+1)):
        ...
        batch_df = pd.DataFrame(data = {'img_name': img_names, 'our_predictions':predictions, 'prob_list': max_sim_list.tolist()})
...
def pred_idx_to_labels(gt_labels, prediction_df):
    ...
    prediction_df['our_predictions'] = prediction_df['our_predictions'].replace(convert_map)
    return prediction_df
